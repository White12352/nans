...
awk '/^replace/ && !found {print "replace github.com/sagernet/sing => ../../sing"; found=1} 1' libcore/.build/src/go.mod > go.mod.tmp && mv -f go.mod.tmp libcore/.build/src/go.mod
awk '/^replace/ && !found {print "replace github.com/sagernet/sing => ../../sing"; found=1} 1' libcore/go.mod > go.mod.tmp && mv -f go.mod.tmp libcore/go.mod
git clone -b dev https://github.com/SagerNet/sing-quic sing-quic
cd sing-box/test
go mod tidy
cd ..
go mod tidy
cd ../libcore/.build/src
go mod tidy
cd ../..
go mod tidy
cd ..
